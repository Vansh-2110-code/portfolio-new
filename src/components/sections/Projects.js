...
const TitleParallaxSolid = styled.h2`
  position: absolute;
  z-index: 5;
  color: ${({ theme }) => theme.name === 'dark' ? 'rgba(255, 255, 255, 0.06)' : 'rgba(0, 0, 0, 0.06)'};
  font-size: clamp(3.5rem, 8.5vw, 8.5rem);
  font-family: 'Anton', sans-serif;
  text-transform: uppercase;
  line-height: 0.9;
...
const ProjectRole = styled.p`
  color: ${({ theme }) => theme.colors.accent || '#818cf8'};
  font-size: 0.75rem;
      ...
      <StickyContainer>
        ...
        <ScrollContainer ref={containerRef}>
          {projects.map((projet, index) => {
            ...
            const watermarkText = translated?.watermark || projet.watermark || title;
