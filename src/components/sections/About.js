...
const contentData = {
  en: {
    default: {
      ...
      desc: "Explore my machine learning stack, enterprise software deployments, and academic achievements. Hover over desk items to interact.",
      icon: "🚀"
    },
    laptop: {
      title: "Machine Learning & AI",
      desc: "Predictive modeling, deep neural architectures with TensorFlow & Scikit-learn, and fraud detection ML models achieving 0.97 AUC.",
      icon: "🧠"
    },
    monitor: {
      title: "Enterprise Cloud & ATS",
      desc: "Full-stack development of Astra CRM, AstraFlow, and a live Applicant Tracking System for Whitehorse Manpower.",
      icon: "🖥️"
    },
    coffee: {
      title: "Data Analytics & Power BI",
      desc: "Transforming complex datasets into actionable business intelligence with Pandas, NumPy, and executive Power BI dashboards.",
      icon: "📊"
    },
    trophy: {
      title: "Honors & Leadership",
      desc: "B.Tech in CS (AI & ML) with 9.45 CGPA, Horizon 8 Hackathon Winner, Chairperson of JU ACM Chapter, and IIT Guwahati certified.",
      icon: "🏆"
    }
  },
  fr: {
    default: {
      ...
      desc: "Explorez ma stack Machine Learning, mes plateformes d'entreprise et mes réalisations. Survolez les éléments du bureau.",
      icon: "🚀"
    },
    laptop: {
      title: "Machine Learning & IA",
      desc: "Modélisation prédictive, réseaux neuronaux avec TensorFlow & Scikit-learn, et modèles anti-fraude atteignant 0.97 AUC.",
      icon: "🧠"
    },
    monitor: {
      title: "Plateformes Entreprise & ATS",
      desc: "Développement d'Astra CRM, AstraFlow et d'un système complet de recrutement (ATS) pour Whitehorse Manpower.",
      icon: "🖥️"
    },
    coffee: {
      title: "Data Analytics & Power BI",
      desc: "Transformation de données brutes en indicateurs stratégiques avec Pandas, NumPy, SQL et tableaux de bord Power BI.",
      icon: "📊"
    },
    trophy: {
      title: "Distinctions & Leadership",
      desc: "Major B.Tech CS (IA & ML) avec 9.45 CGPA, Lauréat Hackathon Horizon 8, Président JU ACM et certifié IIT Guwahati.",
      icon: "🏆"
    }
  },
  ar: {
    default: {
      title: "مساحة العمل التفاعلية",
      desc: "استكشف مهاراتي في تعلم الآلة والأنظمة السحابية والجوائز الأكاديمية عبر تحريك المؤشر والتفاعل مع عناصر المكتب.",
      icon: "🚀"
    },
    laptop: {
      title: "تعلم الآلة والذكاء الاصطناعي",
      desc: "النمذجة التنبؤية، شبكات التعلم العميق مع TensorFlow و Scikit-learn، ونماذج كشف الاحتيال بدقة 0.97 AUC.",
      icon: "🧠"
    },
    monitor: {
      title: "المنصات السحابية وأنظمة ATS",
      desc: "تطوير Astra CRM و AstraFlow ونظام متكامل لتتبع المتقدمين لشركة Whitehorse Manpower.",
      icon: "🖥️"
    },
    coffee: {
      title: "تحليل البيانات و Power BI",
      desc: "تحويل مجموعات البيانات الكبيرة إلى رؤى استراتيجية عبر Pandas و NumPy ولوحات تحكم Power BI.",
      icon: "📊"
    },
    trophy: {
      title: "الجوائز والقيادة",
      desc: "بكالوريوس هندسة الحاسوب (ذكاء اصطناعي وتعلم آلة) بمعدل 9.45، فائز في هاكاثون Horizon 8، ورئيس فرع JU ACM.",
      icon: "🏆"
    }
  }
};
...
export default function About() {
  ...
  const langKey = contentData[language] ? language : 'en';
  const [activeKey, setActiveKey] = useState('default');
  const [hoveredData, setHoveredData] = useState(contentData[langKey].default);
  const [isFading, setIsFading] = useState(false);
  const sceneElementsRef = useRef({});

  useEffect(() => {
    const lk = contentData[language] ? language : 'en';
    setHoveredData(contentData[lk][activeKey] || contentData[lk].default);
  }, [language, activeKey]);

  // Provide a state-based update UI instead of DOM manipulation
  const handleUpdateUI = (dataKey) => {
    const lk = contentData[language] ? language : 'en';
    const data = contentData[lk][dataKey] || contentData[lk].default;
  ...
  useEffect(() => {
    ...
    // LOAD VANSH GOYAL PHOTO AS PNG
    const textureLoader = new THREE.TextureLoader();
    const photoTexture = textureLoader.load('/vansh_photo.png');
  ...
  return (
    <AboutSection id="about" $isRTL={isRTL}>
      ...
      <UILayer>
        <HeaderBox>
          <Badge>{language === 'ar' ? 'سيرة تفاعلية' : language === 'fr' ? 'CV Interactif' : 'Interactive Portfolio'}</Badge>
